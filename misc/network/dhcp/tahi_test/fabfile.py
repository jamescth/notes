...
env.roledefs = {
	...
	'47':['10.25.163.144']			# ddr640-48.chaos.local
}
...
@roles('47')
def tahi_init():
	run('mv /etc/dhclient6-exit-hooks /etc/dhclient6-exit-hooks.ddd')

@roles('47')
def tahi_resume():
	run('mv /etc/dhclient6-exit-hooks.ddd /etc/dhclient6-exit-hooks')

@roles('47')
def tahi_remove_lease():
...
def tahi_iface_up():
	"""
	"""
	run('/sbin/dhclient6 -6 -1 -nw -cf /etc/dhcp/dd_dhclient6.conf -sf /etc/dhclient6-ddr-linux -pf /var/run/dhclient-eth0b.pid eth0b')

@roles('47')
def tahi_iface_down():
	"""
	"""
	run('/sbin/dhclient6 -6 -x -cf /etc/dhcp/dd_dhclient6.conf -sf /etc/dhclient6-ddr-linux -pf /var/run/dhclient-eth0b.pid eth0b')

@roles('47')
def tahi_iface_release():
	"""
	"""
	run('/sbin/dhclient6 -6 -r -cf /etc/dhcp/dd_dhclient6.conf -sf /etc/dhclient6-ddr-linux -pf /var/run/dhclient-eth0b.pid eth0b')
