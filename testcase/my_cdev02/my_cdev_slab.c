...
static int
mycdrv_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_INFO "successfully create a cache object\n");
...
__init my_init(void)
{
	if (size > (1024 * PAGE_SIZE)) {
		printk(KERN_ERR "size=%d is too large; cant have more than 1024 pages\n",
			size);
		return -1;
	}
	...
	printk(KERN_INFO "%s allocated memory cache\n", MYDEV_NAME);
	ramdisk_size = size;
