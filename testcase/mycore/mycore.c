...
print_notes(Elf64_Phdr *myelf)
{
	...
	{
		printf("PT_NOTE   ");
		note_offsets[note_counter] = myelf->p_offset;
		note_counter++;
		break;
	}
...
main(int argc, char *argv[])
{
	...
	buf = mmap(NULL, fd_size, PROT_READ, MAP_FILE|MAP_SHARED, fd, 0);
	if (buf == MAP_FAILED) {
		fprintf(stderr,"mycore: mmap(%s) failed!\n", argv[1]);
		exit(1);
	}
#if 0
// very slow
	...
	/* determine if there is any note recorded contained in the program header */
	if (note_counter != 0) {
		int	note_prcnt;
		unsigned int	note_next_note_off = 0;

		printf("note_counter is %d\n", note_counter);

		// next note offset init 0
		// go through each note section & display all the notes in it 
		for (note_prcnt = 0 ; note_prcnt < note_counter ; note_prcnt++) {
			unsigned int note_sec_off = note_offsets[note_prcnt];
			int	index = 0;
			int	nt_cnt = 0;
