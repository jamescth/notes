...
{
	//dump_state(regs);
	printk(KERN_INFO "pre eax is %lx\n", regs->rax);
	regs->rax = 1;
	return 0;
}

static int
sigabrt_break_handler(struct kprobe *p, struct pt_regs *regs)
{
	printk(KERN_INFO "break eax is %lx\n", regs->rax);
	return 0;
}

static void
sigabrt_post_handler(struct kprobe *p, struct pt_regs *regs,
				       unsigned long flags)
{
	printk(KERN_INFO "post eax is %lx\n", regs->rax);
}

static int
sigabrt_fault_handler(struct kprobe *p, struct pt_regs *regs,
				       int trapnr)
{
	printk(KERN_INFO "fault eax is %lx\n", regs->rax);
	return 0;
}
...
int init_module(void)
{
	...
	kp1.post_handler = sigabrt_post_handler;
	kp1.fault_handler = sigabrt_fault_handler;
	kp1.break_handler = sigabrt_break_handler;
